...
def analyze_note(samples, rate, conditions, *, rendered_rate=None, config=None):
    ...
    analysis_start = min(onset+.08, sustain_end*.5)
    ...
    floor = np.max(spectrum) * 10**(config.peak_floor_db/20)
    ...
    mask = frequencies < ceiling
    centroid = float(np.sum(frequencies[mask]*spectral_power[mask])/np.sum(spectral_power[mask]))
